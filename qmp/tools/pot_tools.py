...
"""
Contains predefined model potentials, has been largely superseded by the
qmp.potential.presets module but can still be used if you would prefer.
"""

import numpy as np
from .dyn_tools import create_real_gaussian, create_real_gaussian2D


def create_potential(cell, name='free', **kwargs):
    """Return 1D potential function.

    Parameters
    ==========
    name : {'free', 'box', 'double_box', 'wall', 'harmonic', 'morse', 'mexican_hat', 'gaussian', 'double_well'}
        Specify the name of the predefined potential that you would like to
        use.
    cell
        Simulation box.
    kwargs:
        Parameters of specific potential (see definitions).
    """
...
def create_potential2D(cell, name='free', **kwargs):
    """Return 2D potential function.

    Parameters
    ==========
    name : {'free', 'box', 'double_box', 'elbow', 'wall', 'harmonic', 'mexican_hat', 'gaussian'}
        Specify the name of the predefined potential that you would like to
        use.
    cell
        Simulation box.
