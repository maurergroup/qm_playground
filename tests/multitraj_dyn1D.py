...
## gather information
r_t = traj1d.data.traj.r_t.flatten().reshape(steps+1,7)
v_t = traj1d.data.traj.v_t.flatten().reshape(steps+1,7)
E_t = traj1d.data.traj.E_t.flatten().reshape(steps+1,7)
E_kin = traj1d.data.traj.E_kin_t.flatten().reshape(steps+1,7)
E_pot = traj1d.data.traj.E_pot_t.flatten().reshape(steps+1,7)
...
wave_plot1, = ax.plot(r_t[0,0], traj1d.pot(r_t[0,0]), label='$r_1(t)$',ls='',marker='o',mfc='r',mec='r',ms=8.5)
wave_plot2, = ax.plot(r_t[0,1], traj1d.pot(r_t[0,1]), label='$r_2(t)$',ls='',marker='o',mfc='k',mec='k',ms=8.5)
...
def animate(i):
    wave_plot1.set_ydata(traj1d.pot(r_t[i,0]))  # update data
    wave_plot1.set_xdata(r_t[i,0])
    wave_plot2.set_ydata(traj1d.pot(r_t[i,1]))  # update data
    wave_plot2.set_xdata(r_t[i,1])
